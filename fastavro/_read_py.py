...
from .six import xrange, btou, utob, iteritems, is_str, str2ints, fstint, long
from .schema import extract_record_type, extract_logical_type, parse_schema
from ._schema_common import SCHEMA_DEFS
from ._read_common import SchemaResolutionError, MAGIC, SYNC_SIZE, HEADER_SCHEMA
...
def read_data(fo, writer_schema, reader_schema=None):
    ...
    if reader_fn:
        ...
        return read_data(fo, SCHEMA_DEFS[record_type], SCHEMA_DEFS.get(reader_schema))
...
class Block:
    ...
    def __init__(
        self, bytes_, num_records, codec, reader_schema, writer_schema, offset, size
    ):
        self.bytes_ = bytes_
    ...
    def __iter__(self):
        for i in xrange(self.num_records):
            yield read_data(self.bytes_, self.writer_schema, self.reader_schema)
...
class file_reader:
    def __init__(self, fo, reader_schema=None):
        ...
        self.metadata = {k: btou(v) for k, v in iteritems(self._header["meta"])}
        ...
        self.writer_schema = parse_schema(self._schema, _write_hint=False, _force=True)
...
class reader(file_reader):
    ...
    def __init__(self, fo, reader_schema=None):
        ...
        self._elems = _iter_avro_records(
            self.fo, self._header, self.codec, self.writer_schema, self.reader_schema
        )
