...
class ValidationErrorData(
    ...
    def __str__(self):
        ...
        if self.datum is None:
            return "Field({field}) is None" " expected {schema}".format(
                field=field, schema=self.schema
            )

        return "{field} is <{datum}> of type " "{given_type} expected {schema}".format(
            datum=self.datum,
            given_type=type(self.datum),
            schema=self.schema,
            field=field,
        )
